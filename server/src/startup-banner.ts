...
import { resolveCombyneConfigPath, resolveCombyneEnvPath } from "./paths.js";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.COMBYNE_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue =
      typeof parsed.COMBYNE_AGENT_JWT_SECRET === "string" ? parsed.COMBYNE_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm combyneai onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveCombyneEnvPath();
  ...
  const art = [
    ...
    color("╚██████╗╚██████╔╝██║ ╚═╝ ██║██████╔╝   ██║   ██║ ╚████║███████╗", "yellow"),
    color(" ╚═════╝ ╚═════╝ ╚═╝     ╚═╝╚═════╝    ╚═╝   ╚═╝  ╚═══╝╚══════╝", "yellow"),
    color("                      .ai — The Hive That Gets Things Done", "dim"),
  ];
  ...
  const lines = [
    ...
    ...(opts.licenseEnabled
      ? [
          row(
            "License",
            opts.licenseStatus === "valid"
              ? `${color("active", "green")} ${color(`(${opts.licensePlanTier ?? "starter"}, until ${opts.licenseValidUntil ?? "?"})`, "dim")}`
              : opts.licenseStatus === "expired"
                ? color("expired", "yellow")
                : opts.licenseStatus === "revoked"
                  ? color("revoked", "yellow")
                  : color("not activated", "yellow"),
          ),
          ...(opts.licenseHeartbeatIntervalMinutes
            ? [row("Lic. Heartbeat", `every ${opts.licenseHeartbeatIntervalMinutes}m`)]
            : []),
        ]
      : []),
