...
import { isCursorUnknownSessionError, parseCursorJsonl } from "@combyne/adapter-cursor-local/server";
...
describe("cursor ui stdout parser", () => {
  ...
  it("compacts shellToolCall and shell tool result for run log", () => {
    const ts = "2026-03-05T00:00:00.000Z";
    const longCommand = "curl -s -X POST \"$COMBYNE_API_URL/api/issues/abc/checkout\" -H \"Authorization: Bearer $COMBYNE_API_KEY\"";
