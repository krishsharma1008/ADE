...
import { execute, testEnvironment } from "@combyne/adapter-openclaw-gateway/server";
import { parseOpenClawGatewayStdoutLine } from "@combyne/adapter-openclaw-gateway/ui";
import type { AdapterExecutionContext } from "@combyne/adapter-utils";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("combyne:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("COMBYNE_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("COMBYNE_TASK_ID=task-123");
