...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Combyne OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Combyne base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("combyneApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.combyneApiUrl");
    expect(text).toContain("~/.openclaw/workspace/combyne-claimed-api-key.json");
    expect(text).toContain("COMBYNE_API_KEY");
