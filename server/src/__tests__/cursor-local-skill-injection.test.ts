...
import { ensureCursorSkillsInjected } from "@combyne/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Combyne skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("combyne-cursor-skills-src-");
    const skillsHome = await makeTempDir("combyne-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "combyne");
    await createSkillDir(skillsDir, "combyne-create-agent");
    ...
    const injectedA = path.join(skillsHome, "combyne");
    const injectedB = path.join(skillsHome, "combyne-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "combyne")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "combyne-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "combyne"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "combyne-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("combyne-cursor-preserve-src-");
    const skillsHome = await makeTempDir("combyne-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "combyne");
    ...
    expect((await fs.lstat(path.join(skillsHome, "combyne-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("combyne-cursor-fail-src-");
    const skillsHome = await makeTempDir("combyne-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
