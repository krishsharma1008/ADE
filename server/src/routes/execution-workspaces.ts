...
import { issues, projects, projectWorkspaces } from "@combyne/db";
import { updateExecutionWorkspaceSchema } from "@combyne/shared";
...
export function executionWorkspaceRoutes(db: Db) {
  ...
  router.post("/execution-workspaces/:id/runtime-services/:action", async (req, res) => {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before Combyne can manage local runtime services" });
      return;
    }
