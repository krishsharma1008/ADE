...
const INVITE_TOKEN_PREFIX = "cmb_invite_";
...
function createClaimSecret() {
  return `cmb_claim_${randomBytes(24).toString("hex")}`;
}
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (normalized !== "combyne" && normalized !== "combyne-create-agent")
    return null;
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.combyneApiUrl)) {
    const legacyCombyneApiUrl = nonEmptyTrimmedString(input.combyneApiUrl);
    if (legacyCombyneApiUrl) merged.combyneApiUrl = legacyCombyneApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    combyneApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.combyneApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawCombyneApiUrl =
    typeof defaults.combyneApiUrl === "string"
      ? defaults.combyneApiUrl.trim()
      : "";
  if (rawCombyneApiUrl) {
    try {
      const parsedCombyneApiUrl = new URL(rawCombyneApiUrl);
      if (
        parsedCombyneApiUrl.protocol !== "http:" &&
        parsedCombyneApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_combyne_api_url_protocol",
          level: "warn",
          message: `combyneApiUrl must use http:// or https:// (got ${parsedCombyneApiUrl.protocol}).`
        });
      } else {
        normalized.combyneApiUrl = parsedCombyneApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_combyne_api_url_configured",
          level: "info",
          message: `combyneApiUrl set to ${parsedCombyneApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_combyne_api_url_invalid",
        level: "warn",
        message: `Invalid combyneApiUrl: ${rawCombyneApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Combyne host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Combyne is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm combyneai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/combyne";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/combyne-claimed-api-key.json and load COMBYNE_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: combyneApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Combyne hostname is reachable and allowed via `pnpm combyneai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Combyne API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/combyne/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Combyne OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          combyneApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Combyne generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "combyneApiUrl": "https://combyne-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Combyne before key claim is allowed.
    ...
    - ~/.openclaw/workspace/combyne-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/combyne-claimed-api-key.json

    And set the COMBYNE_API_KEY and COMBYNE_API_URL in your environment variables as specified here:
    ...
    Then set COMBYNE_API_KEY and COMBYNE_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Combyne skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your COMBYNE_API_URL to the top of your skill and note the path to your COMBYNE_API_URL
    ...
    ${
      ...
      "Ensure Combyne is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Combyne base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.combyneApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm combyneai allowed-hostname <host>
      - then restart Combyne and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@combyne.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "combyne", path: "/api/skills/combyne" },
        {
          name: "combyne-create-agent",
          path: "/api/skills/combyne-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              combyneApiUrl: req.body.combyneApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.combyneApiUrl &&
          !persistedDefaults.combyneApiUrl
        ) {
          missingPersistedFields.push("combyneApiUrl");
        }
