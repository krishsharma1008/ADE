...
import { companies, pluginLogs, pluginWebhookDeliveries } from "@combyne/db";
import type {
  PluginStatus,
...
import type { ToolRunContext } from "@combyne/plugin-sdk";
import { JsonRpcCallError, PLUGIN_RPC_ERROR_CODES } from "@combyne/plugin-sdk";
import { assertBoard, assertCompanyAccess, getActorInfo } from "./authz.js";
import { validateInstanceConfig } from "../services/plugin-config-validator.js";

/** UI slot declaration extracted from plugin manifest */
type PluginUiSlotDeclaration = NonNullable<NonNullable<CombynePluginManifestV1["ui"]>["slots"]>[number];
/** Launcher declaration extracted from plugin manifest */
type PluginLauncherDeclaration = NonNullable<CombynePluginManifestV1["launchers"]>[number];
...
interface PluginInstallRequest {
  /** npm package name (e.g., @combyne/plugin-linear) or local path */
  packageName: string;
...
const BUNDLED_PLUGIN_EXAMPLES: AvailablePluginExample[] = [
  {
    packageName: "@combyne/plugin-hello-world-example",
    pluginKey: "combyne.hello-world-example",
    displayName: "Hello World Widget (Example)",
    description: "Reference UI plugin that adds a simple Hello World widget to the Combyne dashboard.",
    localPath: "packages/plugins/examples/plugin-hello-world-example",
    tag: "example",
  },
  {
    packageName: "@combyne/plugin-file-browser-example",
    pluginKey: "combyne-file-browser-example",
  ...
  {
    packageName: "@combyne/plugin-kitchen-sink-example",
    pluginKey: "combyne-kitchen-sink-example",
    displayName: "Kitchen Sink (Example)",
    description: "Reference plugin that demonstrates the current Combyne plugin API surface, bridge flows, UI extension surfaces, jobs, webhooks, tools, streams, and trusted local workspace/process demos.",
...
export function pluginRoutes(
  db: Db,
  loader?: ReturnType<typeof pluginLoader> | null,
   ...
   *   {
   *     "pluginId": "plg_123",
   *     "pluginKey": "combyne.claude-usage",
  ...
  router.post("/plugins/install", async (req, res) => {
    ...
    try {
      ...
      if (!loader) {
        res.status(503).json({ error: "Plugin loader not initialized" });
        return;
      }
