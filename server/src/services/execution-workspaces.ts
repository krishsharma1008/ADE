...
import { executionWorkspaces, issues, projects, projectWorkspaces, workspaceRuntimeServices } from "@combyne/db";
import type {
  ExecutionWorkspace,
...
async function inspectGitCloseReadiness(workspace: ExecutionWorkspace): Promise<{
  ...
  if (!workspacePath) {
    warnings.push("Workspace has no local path, so Combyne cannot inspect git status before close.");
    return { git: null, warnings };
  }

  if (!(await pathExists(workspacePath))) {
    warnings.push(`Workspace path "${workspacePath}" does not exist, so Combyne cannot inspect git status before close.`);
    return {
      git: {
        repoRoot: null,
        workspacePath,
    ...
    getCloseReadiness: async (id: string): Promise<ExecutionWorkspaceCloseReadiness | null> => {
      ...
      if (executionWorkspace.providerType === "git_worktree" && workspacePath) {
        plannedActions.push({
          ...
          description: `Combyne will run git worktree cleanup for ${workspacePath}.`,
      ...
      if (git?.createdByRuntime && executionWorkspace.branchName) {
        plannedActions.push({
          ...
          description: "Combyne will try to delete the runtime-created branch after removing the worktree.",
      ...
      if (executionWorkspace.providerType === "local_fs" && git?.createdByRuntime && workspacePath) {
        ...
        if (containsProjectWorkspace) {
          warnings.push(`Combyne will archive this workspace but keep "${workspacePath}" because it contains the project workspace.`);
        } else {
          plannedActions.push({
            kind: "remove_local_directory",
            label: "Remove runtime-created directory",
            description: `Combyne will remove the runtime-created directory at ${workspacePath}.`,
