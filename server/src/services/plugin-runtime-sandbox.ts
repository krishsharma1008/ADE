...
import type { CombynePluginManifestV1 } from "@combyne/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __combyne_exports: module.exports,
      __combyne_module: module,
      __combyne_require: requireInSandbox,
      __combyne_filename: realPath,
      __combyne_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__combyne_exports, __combyne_module, __combyne_require, __combyne_filename, __combyne_dirname)`;
