 ...
 *    (`~/.combyne-ai/plugins/`) and `node_modules` for packages matching
 *    the `combyne-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Combyne plugins.
 * Packages matching this pattern are considered Combyne plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "combyne-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: CombynePluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.combyne-ai/plugins/ local directory
  | "npm"               // npm packages matching combyne-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: CombynePluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
   ...
   * Defaults to ~/.combyne-ai/plugins/
   */
  ...
  /**
   * Whether to discover installed npm packages matching the combyne-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "combyne-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: CombynePluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Combyne plugins installed as npm packages in the current
   * Node.js environment matching the "combyne-plugin-*" naming convention.
   *
   ...
   * the "combynePlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Combyne plugin.
   * Throws if the package is a Combyne plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<CombynePluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: CombynePluginManifestV1;
    newManifest: CombynePluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Combyne plugin naming convention.
 * Accepts both the "combyne-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @combyne/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "combynePlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const combynePlugin = pkgJson["combynePlugin"];
  if (
    combynePlugin !== null &&
    typeof combynePlugin === "object" &&
    !Array.isArray(combynePlugin)
  ) {
    const manifestRelPath = (combynePlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: CombynePluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumCombyneVersion;
}
 ...
 *   packageName: "combyne-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: CombynePluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as CombynePluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Combyne plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<CombynePluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Combyne plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasCombynePlugin = "combynePlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasCombynePlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<CombynePluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: CombynePluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @combyne/shared exports). Run those workers through
