 ...
 */
import { pluginManifestV1Schema } from "@combyne/shared";
import type { CombynePluginManifestV1 } from "@combyne/shared";
import { PLUGIN_API_VERSION } from "@combyne/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): CombynePluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as CombynePluginManifestV1,
        };
      }
