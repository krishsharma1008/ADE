...
import { normalizeAgentUrlKey } from "@combyne/shared";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("combyne/combyne/")) return slug;
...
function readCanonicalSkillKey(frontmatter: Record<string, unknown>, metadata: Record<string, unknown> | null) {
  const direct = normalizeSkillKey(
    ...
    ?? asString(metadata?.combyneSkillKey),
  );
  if (direct) return direct;
  const combyne = isPlainRecord(metadata?.combyne) ? metadata?.combyne as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(combyne?.skillKey)
    ?? asString(combyne?.key),
  );
}
...
function deriveCanonicalSkillKey(
  ...
  if (sourceKind === "combyne_bundled") {
    return `combyne/combyne/${slug}`;
  }
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveCombyneInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "combyne_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Combyne skills are read-only.",
      sourceLabel: "Combyne bundled",
      sourceBadge: "combyne",
      sourcePath: null,
    };
  }
...
export function companySkillService(db: Db) {
        ...
        if (!adapter?.listSkills) {
          ...
          try {
            ...
            const snapshot = await adapter.listSkills({
              ...
              config: {
                ...runtimeConfig,
                combyneRuntimeSkills: runtimeSkillEntries,
              },
            });
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "combyne_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled Combyne skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "combyne_bundled"
        && incomingKind === "github"
        && incomingOwner === "combyne"
        && incomingRepo === "combyne"
      ) {
        out.push(existing);
        continue;
      }
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeCombyneSkillSyncPreference(config, filtered),
        });
      }
    }
