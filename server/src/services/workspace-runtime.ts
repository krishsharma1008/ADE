...
import type { AdapterRuntimeServiceReport } from "@combyne/adapter-utils";
import type { Db } from "@combyne/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@combyne/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.COMBYNE_WORKSPACE_CWD = input.worktreePath;
  env.COMBYNE_WORKSPACE_PATH = input.worktreePath;
  env.COMBYNE_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.COMBYNE_WORKSPACE_BRANCH = input.branchName;
  env.COMBYNE_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.COMBYNE_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.COMBYNE_WORKSPACE_SOURCE = input.base.source;
  env.COMBYNE_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.COMBYNE_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.COMBYNE_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.COMBYNE_PROJECT_ID = input.base.projectId ?? "";
  env.COMBYNE_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.COMBYNE_AGENT_ID = input.agent.id ?? "";
  env.COMBYNE_AGENT_NAME = input.agent.name;
  env.COMBYNE_COMPANY_ID = input.agent.companyId;
  env.COMBYNE_ISSUE_ID = input.issue?.id ?? "";
  env.COMBYNE_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.COMBYNE_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.COMBYNE_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.COMBYNE_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.COMBYNE_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.COMBYNE_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.COMBYNE_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.COMBYNE_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.COMBYNE_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.COMBYNE_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.COMBYNE_PROJECT_ID = input.workspace.projectId ?? "";
  env.COMBYNE_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.COMBYNE_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".combyne-ai", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Combyne", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
