...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@combyne/db";
 ...
 *
 * This implementation delegates to the core Combyne domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@combyne/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
