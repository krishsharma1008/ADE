...
import { normalizeAgentUrlKey, portabilityManifestSchema } from "@combyne/shared";
...
async function readAgentInstructions(agent: AgentLike): Promise<{ body: string; warning: string | null }> {
  ...
  if (instructionsFilePath) {
    const workspaceCwd = asString(process.env.COMBYNE_WORKSPACE_CWD);
...
export function companyPortabilityService(db: Db) {
  ...
  async function resolveSource(source: CompanyPortabilityPreview["source"]): Promise<ResolvedSource> {
    ...
    let ref = parsed.ref;
    const manifestRelativePath = [parsed.basePath, "combyne.manifest.json"].filter(Boolean).join("/");
  ...
  function classifyPortableFileKind(filePath: string): CompanyPortabilityExportPreviewFile["kind"] {
    if (filePath.includes("/agents/") || filePath.startsWith("agents/")) return "agent";
    if (filePath.includes("/skills/") || filePath.startsWith("skills/")) return "skill";
    if (filePath.includes("/projects/") || filePath.startsWith("projects/")) return "project";
    if (filePath.includes("/issues/") || filePath.startsWith("issues/")) return "issue";
    if (filePath.endsWith(".combyne.yml") || filePath.endsWith(".combyne.yaml")) return "extension";
    if (/readme/i.test(filePath)) return "readme";
    if (filePath.startsWith("company") || filePath === "manifest.json") return "company";
    return "other";
  }
  ...
  async function previewExport(
    ...
    const exported = await exportBundle(companyId, input);
    return {
      ...exported,
      ...
      counts: {
        ...
        skills: exported.manifest.skills?.length ?? 0,
        projects: exported.manifest.projects?.length ?? 0,
        issues: exported.manifest.issues?.length ?? 0,
      },
    };
  }
