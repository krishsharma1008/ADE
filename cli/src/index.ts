...
const DATA_DIR_OPTION_HELP =
  "Combyne data directory root (isolates state from ~/.combyne)";

program
  .name("combyneai")
  .description("Combyne CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program
  ...
  .option("--run", "Start Combyne immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Combyne setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "combyne")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Combyne")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Combyne server API")
