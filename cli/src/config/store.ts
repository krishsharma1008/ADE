...
import { combyneConfigSchema, type CombyneConfig } from "./schema.js";
import {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".combyne", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  if (process.env.COMBYNE_CONFIG) return path.resolve(process.env.COMBYNE_CONFIG);
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveCombyneInstanceId());
}
...
export function readConfig(configPath?: string): CombyneConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = combyneConfigSchema.safeParse(migrated);
  if (!parsed.success) {
