...
function tempConfigPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "combyne-jwt-env-"));
...
describe("agent jwt env helpers", () => {
  beforeEach(() => {
    ...
    delete process.env.COMBYNE_AGENT_JWT_SECRET;
  });
  ...
  it("writes .env next to explicit config path", () => {
    ...
    expect(contents).toContain("COMBYNE_AGENT_JWT_SECRET=");
  });

  it("loads secret from .env next to explicit config path", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "COMBYNE_AGENT_JWT_SECRET=test-secret\n", { mode: 0o600 });

    const loaded = readAgentJwtSecretFromEnv(configPath);
    expect(loaded).toBe("test-secret");
    expect(process.env.COMBYNE_AGENT_JWT_SECRET).toBe("test-secret");
  });

  it("doctor check passes when secret exists in adjacent .env", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "COMBYNE_AGENT_JWT_SECRET=check-secret\n", { mode: 0o600 });
