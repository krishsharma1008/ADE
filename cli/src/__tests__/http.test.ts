...
describe("CombyneApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new CombyneApiClient({
    ...
    expect(headers["x-combyne-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new CombyneApiClient({ apiBase: "http://localhost:3100" });
