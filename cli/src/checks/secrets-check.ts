...
export function secretsCheck(config: CombyneConfig, configPath?: string): CheckResult {
  ...
  if (provider !== "local_encrypted") {
    return {
      ...
      canRepair: false,
      repairHint: "Run `combyneai configure --section secrets` and set provider to local_encrypted",
    };
  }

  const envMasterKey = process.env.COMBYNE_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    if (!decodeMasterKey(envMasterKey)) {
      return {
        ...
        status: "fail",
        message:
          "COMBYNE_SECRETS_MASTER_KEY is invalid (expected 32-byte base64, 64-char hex, or raw 32-char string)",
        canRepair: false,
        repairHint: "Set COMBYNE_SECRETS_MASTER_KEY to a valid key or unset it to use a key file",
      };
    }
    ...
    return withStrictModeNote(
      {
        name: "Secrets adapter",
        status: "pass",
        message: "Local encrypted provider configured via COMBYNE_SECRETS_MASTER_KEY",
      },
      config,
    );
  }

  const keyFileOverride = process.env.COMBYNE_SECRETS_MASTER_KEY_FILE;
  ...
  try {
    ...
    return {
      ...
      canRepair: false,
      repairHint: "Check file permissions or set COMBYNE_SECRETS_MASTER_KEY",
    };
  }
