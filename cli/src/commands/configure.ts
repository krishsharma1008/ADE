...
function defaultConfig(): CombyneConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" combyne configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `combyneai onboard` first.");
    p.outro("");
    return;
  }

  let config: CombyneConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because COMBYNE_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
