...
function buildAgentEnvExports(input: {
  ...
  return [
    `export COMBYNE_API_URL='${escaped(input.apiBase)}'`,
    `export COMBYNE_COMPANY_ID='${escaped(input.companyId)}'`,
    `export COMBYNE_AGENT_ID='${escaped(input.agentId)}'`,
    `export COMBYNE_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Combyne skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Combyne skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            ...
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Combyne skills directory. Expected ./skills in the repo checkout.",
              );
            }
