...
import { ApiRequestError, CombyneApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Combyne config file")
    .option("-d, --data-dir <path>", "Combyne data directory root (isolates state from ~/.combyne)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Combyne API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.COMBYNE_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.COMBYNE_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.COMBYNE_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set COMBYNE_COMPANY_ID, or set context profile companyId via `combyneai context set`.",
    );
  }

  const api = new CombyneApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.COMBYNE_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.COMBYNE_SERVER_PORT || "");
