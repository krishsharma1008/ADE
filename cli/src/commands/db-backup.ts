...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@combyne/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://combyne:combyne@127.0.0.1:${port}/combyne`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" combyne db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveCombyneInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "combyne";
