...
import { printCombyneCliBanner } from "../utils/banner.js";
...
type OnboardDefaults = Pick<CombyneConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "COMBYNE_PUBLIC_URL",
  "DATABASE_URL",
  "COMBYNE_DB_BACKUP_ENABLED",
  "COMBYNE_DB_BACKUP_INTERVAL_MINUTES",
  "COMBYNE_DB_BACKUP_RETENTION_DAYS",
  "COMBYNE_DB_BACKUP_DIR",
  "COMBYNE_DEPLOYMENT_MODE",
  "COMBYNE_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "COMBYNE_ALLOWED_HOSTNAMES",
  "COMBYNE_AUTH_BASE_URL_MODE",
  "COMBYNE_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "COMBYNE_STORAGE_PROVIDER",
  "COMBYNE_STORAGE_LOCAL_DIR",
  "COMBYNE_STORAGE_S3_BUCKET",
  "COMBYNE_STORAGE_S3_REGION",
  "COMBYNE_STORAGE_S3_ENDPOINT",
  "COMBYNE_STORAGE_S3_PREFIX",
  "COMBYNE_STORAGE_S3_FORCE_PATH_STYLE",
  "COMBYNE_SECRETS_PROVIDER",
  "COMBYNE_SECRETS_STRICT_MODE",
  "COMBYNE_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveCombyneInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.COMBYNE_PUBLIC_URL?.trim() ||
    process.env.COMBYNE_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.COMBYNE_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.COMBYNE_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.COMBYNE_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.COMBYNE_ALLOWED_HOSTNAMES
    ? process.env.COMBYNE_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.COMBYNE_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.COMBYNE_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.COMBYNE_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.COMBYNE_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.COMBYNE_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.COMBYNE_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.COMBYNE_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.COMBYNE_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.COMBYNE_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.COMBYNE_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.COMBYNE_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.COMBYNE_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.COMBYNE_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.COMBYNE_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.COMBYNE_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "COMBYNE_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<CombyneConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printCombyneCliBanner();
  p.intro(pc.bgCyan(pc.black(" combyneai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveCombyneInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: CombyneConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        ...
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `combyneai doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("COMBYNE_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.COMBYNE_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("COMBYNE_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("COMBYNE_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }

  const config: CombyneConfig = {
    $meta: {
      version: 1,
  ...
  p.note(
    [
      ...
      "Agent auth: COMBYNE_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("combyneai run")}`,
      `Reconfigure later: ${pc.cyan("combyneai configure")}`,
      `Diagnose setup: ${pc.cyan("combyneai doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.COMBYNE_OPEN_ON_LISTEN = "true";
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("combyneai run")}`,
        `Then: ${pc.cyan("combyneai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
