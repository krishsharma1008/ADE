...
import { createDb, instanceUserRoles, invites } from "@combyne/db";
...
function createInviteToken() {
  return `cmb_bootstrap_${randomBytes(24).toString("hex")}`;
}
...
function resolveDbUrl(configPath?: string, explicitDbUrl?: string) {
  ...
  if (config?.database.mode === "embedded-postgres") {
    ...
    return `postgres://combyne:combyne@127.0.0.1:${port}/combyne`;
  }
  return null;
}
...
export async function bootstrapCeoInvite(opts: {
  ...
  const config = readConfig(configPath);
  if (!config) {
    p.log.error(`No config found at ${configPath}. Run ${pc.cyan("combyne onboard")} first.`);
    return;
  }
  ...
  try {
    ...
    p.log.info("If using embedded-postgres, start the Combyne server and run this command again.");
  }
}
