...
function findContextFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".combyne", DEFAULT_CONTEXT_BASENAME);
...
export function resolveContextPath(overridePath?: string): string {
  ...
  if (process.env.COMBYNE_CONTEXT) return path.resolve(process.env.COMBYNE_CONTEXT);
