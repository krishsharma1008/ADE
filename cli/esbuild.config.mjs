/**
 * esbuild configuration for building the combyne CLI for npm.
 *
 * Bundles all workspace packages (@combyne/*) into a single file.
...
// to npm and resolved at runtime (e.g. @combyne/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@combyne/")) {
      externals.add(name);
    }
  }
