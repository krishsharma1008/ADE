...
const COMBYNE_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];
...
export function buildCombyneEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.COMBYNE_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort =
    process.env.COMBYNE_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl =
    process.env.COMBYNE_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.COMBYNE_API_URL = apiUrl;
  return vars;
}
...
export function defaultPathForPlatform() {
  ...
  const extras = home
    ? `${home}/.npm-global/bin:${home}/.local/bin:${home}/.nvm/versions/node/current/bin:`
    : "";
  return `${extras}/usr/local/bin:/opt/homebrew/bin:/usr/local/sbin:/usr/bin:/bin:/usr/sbin:/sbin`;
}
...
export async function ensureAbsoluteDirectory(
  ...
  // Strip surrounding quotes that may have been entered in the UI
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Combyne server itself is started from
    // within a Claude Code session (e.g. `npx combyne run` in a terminal
...
// ---------------------------------------------------------------------------
// Skill management utilities (ported from Paperclip adapter-utils)
// ---------------------------------------------------------------------------

export interface CombyneSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: CombyneSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "combyne_required",
      originLabel: "Required by Combyne",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Combyne",
    readOnly: false,
  };
}
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "COMBYNE_RESOLVED_COMMAND"] = resolvedCommand;
  }
...
export async function resolveCombyneSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...COMBYNE_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listCombyneSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<CombyneSkillEntry[]> {
  const root = await resolveCombyneSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `combyne/combyne/${entry.name}`,
        runtimeName: entry.name,
        source: path.join(root, entry.name),
        required: true,
        requiredReason: "Bundled Combyne skills are always available for local adapters.",
      }));
  } catch {
    return [];
  }
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Combyne skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Combyne cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredCombyneRuntimeSkills(value: unknown): CombyneSkillEntry[] {
...
export async function readCombyneRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<CombyneSkillEntry[]> {
  const configuredEntries = normalizeConfiguredCombyneRuntimeSkills(config.combyneRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listCombyneSkillEntries(moduleDir, additionalCandidates);
}

export async function readCombyneSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listCombyneSkillEntries(moduleDir);
...
export function readCombyneSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.combyneSkillSync;
...
function canonicalizeDesiredCombyneSkillReference(
  reference: string,
...
export function resolveCombyneDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readCombyneSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredCombyneSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeCombyneSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.combyneSkillSync;
  const current =
  ...
  next.combyneSkillSync = current;
  return next;
}

export async function ensureCombyneSkillSymlink(
  source: string,
  target: string,
