...
function resolveCombyneHomeDir(): string {
  const envHome = process.env.COMBYNE_HOME?.trim();
  if (envHome) return path.resolve(expandHomePrefix(envHome));
  return path.resolve(os.homedir(), ".combyne");
}

function resolveCombyneInstanceId(): string {
  const raw = process.env.COMBYNE_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveCombyneHomeDir(), "instances", resolveCombyneInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveCombyneHomeDir(), "instances", resolveCombyneInstanceId(), "data", "backups");
}
