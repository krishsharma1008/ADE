...
export const heartbeatRuns = pgTable(
  "heartbeat_runs",
  {
    ...
    processHostId: text("process_host_id"),
    processStartedAt: timestamp("process_started_at", { withTimezone: true }),
    processLastHeartbeatAt: timestamp("process_last_heartbeat_at", { withTimezone: true }),
    processRestartGeneration: integer("process_restart_generation").notNull().default(0),
    recoveryStatus: text("recovery_status"),
    retryOfRunId: uuid("retry_of_run_id"),
    processLossRetryCount: integer("process_loss_retry_count").notNull().default(0),
    contextSnapshot: jsonb("context_snapshot").$type<Record<string, unknown>>(),
    promptBudgetJson: jsonb("prompt_budget_json").$type<Record<string, unknown>>(),
