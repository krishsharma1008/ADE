...
async function buildGeminiSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  const availableEntries = await readCombyneRuntimeSkillEntries(config, __moduleDir);
  const desiredSkills = resolveCombyneDesiredSkillNames(config, availableEntries);
  ...
  return buildPersistentSkillSnapshot({
    ...
    externalDetail: "Installed outside Combyne management.",
  });
}
...
export async function syncGeminiSkills(
  ...
  const availableEntries = await readCombyneRuntimeSkillEntries(ctx.config, __moduleDir);
  ...
  for (const available of availableEntries) {
    ...
    await ensureCombyneSkillSymlink(available.source, target);
  }
...
export function resolveGeminiDesiredSkillNames(
  ...
  return resolveCombyneDesiredSkillNames(config, availableEntries);
}
