...
Use when:
- You want Combyne to run Pi (the AI coding agent) locally as the agent runtime
...
- Combyne requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/combynes/ and resumed with --session.
