...
import type { AdapterModel } from "@combyne/adapter-utils";
import { asString, runChildProcess } from "@combyne/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.COMBYNE_PI_COMMAND === "string" &&
    process.env.COMBYNE_PI_COMMAND.trim().length > 0
      ? process.env.COMBYNE_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["COMBYNE_", "npm_", "NPM_"] as const;
