...
export async function ensureCursorSkillsInjected(
  ...
  const skillsDir = options.skillsDir ?? await resolveCombyneSkillsDir();
  if (!skillsDir) return;
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[combyne] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[combyne] Failed to read Combyne skills from ${skillsDir}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[combyne] Injected Cursor skill "${entry.name}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[combyne] Failed to inject Cursor skill "${entry.name}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[combyne] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (instructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[combyne] Loaded agent instructions file: ${instructionsFilePath}\n`,
      );
    } catch (err) {
  ...
  const prompt = `${instructionsPrefix}${combyneEnvNote}${renderedPrompt}`;
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[combyne] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
