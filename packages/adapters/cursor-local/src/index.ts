...
Use when:
- You want Combyne to run Cursor Agent CLI locally as the agent runtime
...
- Combyne auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$combyne" and related skills on local runs.
- Combyne auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
