...
## Scope
This plan is now **gateway-only**. Combyne supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Combyne skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Combyne task.
...
```bash
pnpm --filter @combyne/server test:run -- openclaw-gateway
pnpm --filter @combyne/server typecheck
pnpm --filter @combyne/ui typecheck
pnpm --filter combyne typecheck
```
