...
import { asString, parseObject } from "@combyne/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Combyne server host.",
        });
      }
    } catch (err) {
      checks.push({
