...
import { asNumber, asString, buildCombyneEnv, parseObject } from "@combyne/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "combyne";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "combyne";
  if (input.strategy === "run") return `combyne:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `combyne:issue:${input.issueId}`;
  return fallback;
}
...
function resolveCombyneApiUrlOverride(value: unknown): string | null {
...
function buildCombyneEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const combyneApiUrlOverride = resolveCombyneApiUrlOverride(ctx.config.combyneApiUrl);
  const combyneEnv: Record<string, string> = {
    ...buildCombyneEnv(ctx.agent),
    COMBYNE_RUN_ID: ctx.runId,
  };
  // After all COMBYNE_* vars are set, apply any additional overrides

  if (combyneApiUrlOverride) {
    combyneEnv.COMBYNE_API_URL = combyneApiUrlOverride;
  }
  if (wakePayload.taskId) combyneEnv.COMBYNE_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) combyneEnv.COMBYNE_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) combyneEnv.COMBYNE_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) combyneEnv.COMBYNE_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) combyneEnv.COMBYNE_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    combyneEnv.COMBYNE_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return combyneEnv;
}

function buildWakeText(payload: WakePayload, combyneEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/combyne-claimed-api-key.json";
  const orderedKeys = [
    "COMBYNE_RUN_ID",
    "COMBYNE_AGENT_ID",
    "COMBYNE_COMPANY_ID",
  ...
  for (const key of orderedKeys) {
    const value = combyneEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = combyneEnv.COMBYNE_API_URL ?? "<set COMBYNE_API_URL>";

  const lines = [
    "Combyne wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `COMBYNE_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load COMBYNE_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $COMBYNE_API_KEY on every API call.",
    "- Use X-Combyne-Run-Id: $COMBYNE_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: COMBYNE_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$COMBYNE_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$COMBYNE_COMPANY_ID/issues?assigneeAgentId=$COMBYNE_AGENT_ID&status=todo,in_progress,blocked",
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "combyne-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const combyneEnv = buildCombyneEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, combyneEnv);
