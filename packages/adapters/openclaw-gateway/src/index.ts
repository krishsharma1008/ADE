...
Use when:
- You want Combyne to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Combyne server.
...
- combyneApiUrl (string, optional): absolute Combyne base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default combyne)
`;
