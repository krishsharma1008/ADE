...
type GlobalBridge = typeof globalThis & {
  __combynePluginBridge__?: PluginBridgeRegistry;
};

function getBridgeRegistry(): PluginBridgeRegistry | undefined {
  return (globalThis as GlobalBridge).__combynePluginBridge__;
}

function missingBridgeValueError(name: string): Error {
  return new Error(
    `Combyne plugin UI runtime is not initialized for "${name}". ` +
...
export function renderSdkUiComponent<TProps>(
  ...
  throw new Error(`Combyne plugin UI component "${name}" is not callable`);
}
