/**
 * `@combyne/plugin-sdk` — Combyne plugin worker-side SDK.
 *
 * This is the main entrypoint for plugin worker code.  For plugin UI bundles,
 * import from `@combyne/plugin-sdk/ui` instead.
 *
 * @example
 * ```ts
 * // Plugin worker entrypoint (dist/worker.ts)
 * import { definePlugin, runWorker, z } from "@combyne/plugin-sdk";
 *
...
// Manifest and constant types re-exported from @combyne/shared
// Plugin authors import manifest types from here so they have a single
// dependency (@combyne/plugin-sdk) for all plugin authoring needs.
export type {
 ...
 * @example
 * ```ts
 * import { z } from "@combyne/plugin-sdk";
 *
