/**
 * Core types for the Combyne plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@combyne/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @combyne/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Combyne object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Combyne UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Combyne domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Combyne domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Combyne secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Combyne secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@combyne/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@combyne/plugin-sdk";
 *
