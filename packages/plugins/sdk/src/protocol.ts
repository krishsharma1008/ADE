 ...
 * channel.
 *
 * The Combyne plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@combyne/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all Combyne messages.
 */
...
/**
 * Combyne plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this Combyne instance. */
    instanceId: string;
    /** Semver version of the running Combyne host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
