...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".combyne-sdk");
...
/**
 * Generate a complete Combyne plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Combyne plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "combyne-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["combyne", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@combyne/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@combyne/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import type { CombynePluginManifestV1 } from "@combyne/plugin-sdk";

const manifest: CombynePluginManifestV1 = {
  id: ${quote(manifestId)},
  apiVersion: 1,
  version: "0.1.0",
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@combyne/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@combyne/plugin-sdk\` and \`@combyne/shared\` from a local Combyne checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.combyne-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into Combyne
...
- \`pnpm build\` uses esbuild presets from \`@combyne/plugin-sdk/bundlers\`.
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-combyne-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <combyne-sdk-path>]");
    process.exit(1);
  }
