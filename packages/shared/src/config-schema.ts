...
export const databaseBackupConfigSchema = z.object({
  ...
  dir: z.string().default("~/.combyne/instances/default/data/backups"),
});

export const databaseConfigSchema = z.object({
  mode: z.enum(["embedded-postgres", "postgres"]).default("embedded-postgres"),
  connectionString: z.string().optional(),
  embeddedPostgresDataDir: z.string().default("~/.combyne/instances/default/db"),
  embeddedPostgresPort: z.number().int().min(1).max(65535).default(54329),
  backup: databaseBackupConfigSchema.default({
    enabled: true,
    intervalMinutes: 60,
    retentionDays: 30,
    dir: "~/.combyne/instances/default/data/backups",
  }),
});

export const loggingConfigSchema = z.object({
  mode: z.enum(["file", "cloud"]),
  logDir: z.string().default("~/.combyne/instances/default/logs"),
});
...
export const storageLocalDiskConfigSchema = z.object({
  baseDir: z.string().default("~/.combyne/instances/default/data/storage"),
});

export const storageS3ConfigSchema = z.object({
  bucket: z.string().min(1).default("combyne"),
...
export const storageConfigSchema = z.object({
  ...
  localDisk: storageLocalDiskConfigSchema.default({
    baseDir: "~/.combyne/instances/default/data/storage",
  }),
...
export const secretsLocalEncryptedConfigSchema = z.object({
  keyFilePath: z.string().default("~/.combyne/instances/default/secrets/master.key"),
});
...
export const secretsConfigSchema = z.object({
  ...
  localEncrypted: secretsLocalEncryptedConfigSchema.default({
    keyFilePath: "~/.combyne/instances/default/secrets/master.key",
  }),
});

export const licenseConfigSchema = z.object({
  enabled: z.boolean().default(false),
  supabaseUrl: z.string().url().default("https://cmkybsmznmhclytbjnwh.supabase.co"),
  supabaseAnonKey: z.string().default(""),
  heartbeatIntervalMinutes: z.number().int().min(15).max(1440).default(60),
  gracePeriodHours: z.number().int().min(1).max(168).default(24),
});

export const combyneConfigSchema = z
  .object({
    ...
    license: licenseConfigSchema.optional(),
  })
...
export type CombyneConfig = z.infer<typeof combyneConfigSchema>;
