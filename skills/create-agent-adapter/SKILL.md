...
description: >
  Technical guide for creating a new Combyne agent adapter. Use when building
...
# Creating a Combyne Agent Adapter

An adapter bridges Combyne's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `combyneai run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@combyneai/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@combyneai/adapter-utils` (types) or `@combyneai/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell Combyne to forget the stored session
}
...
```json
{
  ...
  "dependencies": {
    "@combyneai/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
The `agentConfigurationDoc` is read by LLM agents (including Combyne agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@combyneai/adapter-utils/server-utils`)
2. **Build environment** — call `buildCombyneEnv(agent)` then layer in `COMBYNE_RUN_ID`, context vars (`COMBYNE_TASK_ID`, `COMBYNE_WAKE_REASON`, `COMBYNE_WAKE_COMMENT_ID`, `COMBYNE_APPROVAL_ID`, `COMBYNE_APPROVAL_STATUS`, `COMBYNE_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| Variable | Source |
|----------|--------|
| `COMBYNE_AGENT_ID` | `agent.id` |
| `COMBYNE_COMPANY_ID` | `agent.companyId` |
| `COMBYNE_API_URL` | Server's own URL |
| `COMBYNE_RUN_ID` | Current run id |
| `COMBYNE_TASK_ID` | `context.taskId` or `context.issueId` |
| `COMBYNE_WAKE_REASON` | `context.wakeReason` |
| `COMBYNE_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `COMBYNE_APPROVAL_ID` | `context.approvalId` |
| `COMBYNE_APPROVAL_STATUS` | `context.approvalStatus` |
| `COMBYNE_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `COMBYNE_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `combyneai run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@combyneai/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@combyneai/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use Combyne API
...
import { parseMyAgentStdoutLine } from "@combyneai/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@combyneai/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@combyneai/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so Combyne wipes the stale session
...
Import from `@combyneai/adapter-utils/server-utils`:
...
| `buildCombyneEnv(agent)` | Standard `COMBYNE_*` env vars |
...
- Package name: `@combyneai/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your Combyne work."`
...
### Combyne Skills Injection

Combyne ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `combyne` API skill and the `combyne-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with Combyne internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("combyne-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  ...
  const entries = await fs.readdir(COMBYNE_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(COMBYNE_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks Combyne skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
- `COMBYNE_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the Combyne API and GitHub should not have access to arbitrary hosts.
