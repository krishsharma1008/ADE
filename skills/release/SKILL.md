...
name: release
description: >
  Coordinate a full Combyne release across engineering verification, npm,
...
Run the full Combyne release as a maintainer workflow, not just an npm publish.
...
7. If running through Combyne, you have issue context for status updates and follow-up task creation.
...
Combyne now uses this release model:
...
```bash
npm view combyne@canary version
```
...
- any release summary in Combyne issue context
