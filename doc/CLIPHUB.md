...
ClipHub is the public registry where people share, discover, and download Combyne company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Combyne what a package registry is to a programming language. Combyne already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Combyne — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Combyne instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Combyne spec. It contains:
...
**Install (fresh start):**
```
combyne install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Combyne instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Combyne, export your company as a template, then publish:

```
combyne export --template my-company
combyne publish cliphub my-company
```
...
ClipHub is a **separate service** from Combyne itself. Combyne is self-hosted; ClipHub is a hosted registry that Combyne instances talk to.
...
| **Combyne CLI** | `combyne-ai install`, `combyne-ai publish`, `combyne-ai cliphub sync` — built into Combyne |
| **Combyne UI** | "Browse ClipHub" panel in the Combyne web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Combyne) |
| Backend | TypeScript + Hono (consistent with Combyne) |
| Database | PostgreSQL |
...
4. Run `combyne-ai install cliphub:acme/lean-saas-shop`
5. Combyne creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Combyne until it works well
2. Export: `combyne-ai export --template my-agency`
3. Publish: `combyne-ai publish cliphub my-agency`
...
3. Install just that agent: `combyne-ai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Combyne

ClipHub is **not required** to use Combyne. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Combyne what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `combyne-ai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Combyne web UI
- [ ] `combyne-ai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Combyne's own philosophy)
