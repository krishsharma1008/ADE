...
Maintainer runbook for shipping a full Combyne release across npm, GitHub, and the website-facing changelog surface.
...
Every Combyne release has four separate surfaces:

1. **Verification** — the exact git SHA must pass typecheck, tests, and build.
2. **npm** — `combyne-ai` and the public workspace packages are published.
...
VERSION=0.2.8
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Combyne. Read doc/RELEASING.md and skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
...
COMBYNE_VERSION=canary ./scripts/docker-onboard-smoke.sh

# Users install with:
npx combyne@canary onboard
```
...
```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary COMBYNE_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable COMBYNE_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

This is the best existing fit when you want:

- a standalone Combyne data dir
- a dedicated host port
- an end-to-end `npx combyne ... onboard` check
...
COMBYNE_PORT=3234 ./scripts/clean-onboard-ref.sh
...
- [ ] You are comfortable with users installing it via `npx combyne@canary onboard`
...
- [ ] `npm view combyne-ai@latest version` matches the new stable version
...
```bash
VERSION=X.Y.Z
claude -p "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Combyne. Read doc/RELEASING.md and skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
- [ ] `npx combyne@canary onboard` installs
...
- users need `npx combyne onboard` to recover quickly
...
3. verify `npm view combyne-ai@latest version`
