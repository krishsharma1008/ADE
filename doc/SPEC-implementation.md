# Combyne V1 Implementation Spec
...
Combyne V1 must provide a full control-plane loop for autonomous agents:
...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.combyne-ai/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.combyne-ai/instances/default/data/storage` (`local_disk`)
...
- exactly one JSON entrypoint: `combyne.manifest.json`
