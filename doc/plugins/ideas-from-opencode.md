...
Combyne V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Combyne-shaped design.

Assumption for this document: Combyne is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Combyne objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Combyne.

The main conclusion is:

- Combyne should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Combyne should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Combyne should use multiple extension classes instead of one generic plugin bag:
...
If Combyne does this well, the examples you listed become straightforward:
...
Relevant Combyne files reviewed for current extension seams:
...
It is too dangerous for Combyne core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Combyne — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Combyne should absolutely do this.
...
Combyne should copy this discipline.
...
Combyne should adopt this style for plugin actions, automations, and UI schemas.
...
Combyne can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Combyne too.

## What Combyne Should Not Copy Directly
...
Combyne is a control plane for an operator-managed instance with company objects.
...
Combyne is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Combyne should not auto-load arbitrary code from a workspace repo like `.combyne/plugins` or project directories.
...
For Combyne, equivalent hooks into:
...
Combyne should not allow plugins to silently replace:
...
For Combyne it would be risky because it combines:
...
Combyne should require an explicit operator install step.

## Why Combyne Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Combyne |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Combyne instance |
...
That means Combyne should borrow the good ideas from `opencode` but use a stricter architecture.

## Combyne Already Has Useful Pre-Plugin Seams

Combyne has several extension-like seams already:
...
This is good news.
Combyne does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Combyne Plugin Model
...
Combyne already has this pattern implicitly:
...
- `plugin` means integration code that talks to Combyne through a typed plugin protocol and capability model
...
- invoke explicit Combyne actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@combyne/plugin-sdk/ui`) with design tokens and common components.
...
Combyne should treat plugin installation as a global instance-level action.

Examples:

- install `@combyne/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Combyne objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Combyne already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Combyne too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@combyne/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@combyne/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Combyne instance.

A `create-combyne-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Combyne achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Combyne server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Combyne's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@combyne/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@combyne/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Combyne

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@combyne/plugin-sdk";

export default definePlugin({
  id: "@combyne/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Combyne entities
    });
    ...
    ctx.events.on("plugin.@combyne/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@combyne/plugin-sdk/ui";
...
Any Combyne plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Combyne objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@combyne/plugin-linear`)
...
- reuse core tables when the data is clearly part of Combyne itself
...
If a concept is becoming part of Combyne's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Combyne itself
...
### 3. Scope plugin data to Combyne objects before adding custom schemas

A lot of plugin data naturally hangs off existing Combyne objects:
...
If Combyne eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@combyne/plugin-workspace-files`
...
Package idea: `@combyne/plugin-terminal`
...
Package idea: `@combyne/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Combyne.
- Board reviews the diff after a run without leaving Combyne.
...
- optional `events.emit` (e.g. `plugin.@combyne/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@combyne/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@combyne/plugin-linear`

This plugin syncs Combyne work with Linear. It is useful for:

- importing backlog from Linear
- linking Combyne issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Combyne and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Combyne.
- Combyne issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@combyne/plugin-github-issues`

This plugin syncs Combyne issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Combyne issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Combyne task governance
...
- Plugin settings:
  ...
  - whether new Combyne issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Combyne issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Combyne.
- GitHub webhooks update status/comment state in Combyne.
- A PR is linked back to the Combyne issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@combyne/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@combyne/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Combyne. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Combyne issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Combyne plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Combyne dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Combyne issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Combyne core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@combyne/plugin-runtime-processes`
...
Package idea: `@combyne/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Combyne. It is useful for:
...
- Failed payment spikes or churn events can generate Combyne issues for follow-up.
...
- Stripe data should stay additive to Combyne core
...
- plugin UI bundle loading, host bridge, `@combyne/plugin-sdk/ui`
...
- `@combyne/plugin-test-harness` and `create-combyne-plugin` starter template
...
Combyne should not implement "an OpenCode-style generic in-process hook system."
Combyne should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Combyne
...
8. Ship `@combyne/plugin-test-harness` and `create-combyne-plugin` starter template.
