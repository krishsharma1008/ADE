# Combyne Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Combyne's plugin and extension architecture.
...
Combyne plugin design is based on the following assumptions:

1. Combyne is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Combyne business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Combyne core.
...
2. Let plugins add major capabilities without editing Combyne core.
...
4. Depend on project-local plugin folders such as `.combyne/plugins`.
...
The single Combyne deployment an operator installs and controls.

### 5.2 Company

A first-class Combyne business object inside the instance.
...
A trusted in-process extension loaded directly by Combyne core.
...
An installable instance-wide extension package loaded through the Combyne plugin runtime.
...
Combyne has two extension classes.
...
Combyne already has a concrete workspace model:
...
Plugins live under the Combyne instance directory.

Suggested layout:

- `~/.combyne-ai/instances/default/plugins/package.json`
- `~/.combyne-ai/instances/default/plugins/node_modules/`
- `~/.combyne-ai/instances/default/plugins/.cache/`
- `~/.combyne-ai/instances/default/data/plugins/<plugin-id>/`

The package install directory and the plugin data directory are separate.

## 8.2 Operator Commands

Combyne should add CLI commands:

- `pnpm combyne plugin list`
- `pnpm combyne plugin install <package[@version]>`
- `pnpm combyne plugin uninstall <plugin-id>`
- `pnpm combyne plugin upgrade <plugin-id> [version]`
- `pnpm combyne plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@combyne/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface CombynePluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Combyne agents can use during runs.
...
- Combyne server starts one worker process per installed plugin
...
Receives one typed Combyne domain event.
...
export function definePlugin(definition: PluginDefinition): CombynePlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: CombynePluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@combyne/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@combyne/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@combyne/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@combyne/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@combyne/plugin-sdk/ui`
...
The `@combyne/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Combyne secret provider system rather than a plain text input
...
1. Core Combyne data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Combyne objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Combyne product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Combyne later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Combyne secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm combyne plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Combyne server. This is a normative requirement, not optional.
...
### 27.1 `@combyne/plugin-test-harness`
...
```ts
import { createTestHarness } from "@combyne/plugin-test-harness";
...
For developing a plugin against a running Combyne instance:

- The operator installs the plugin from a local path: `pnpm combyne plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-combyne-plugin`) that scaffolds:

- `package.json` with correct `combynePlugin` keys
...
- `@combyne/plugin-workspace-files`
- `@combyne/plugin-terminal`
- `@combyne/plugin-git`
- `@combyne/plugin-linear`
- `@combyne/plugin-github-issues`
- `@combyne/plugin-grafana`
- `@combyne/plugin-runtime-processes`
- `@combyne/plugin-stripe`
...
- `@combyne/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@combyne/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@combyne/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@combyne/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@combyne/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@combyne/plugin-sdk/ui`
...
- `@combyne/plugin-test-harness`
- `create-combyne-plugin` starter template
...
Combyne should not implement a generic in-process hook bag modeled directly after local coding tools.

Combyne should implement:
...
That is the complete target design for the Combyne plugin system.
