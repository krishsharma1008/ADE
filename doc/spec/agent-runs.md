...
This spec defines how Combyne actually runs agents while staying runtime-agnostic.
...
1. Combyne is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Combyne manages lifecycle, and on exit Combyne parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `combyne.skill` (shared Combyne skill text block)
...
2. Preferred transport is env vars (`COMBYNE_*`) injected at runtime.
