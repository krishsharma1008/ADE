...
- New config/env settings:
  - `COMBYNE_AGENT_JWT_SECRET`
  - `COMBYNE_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `COMBYNE_AGENT_JWT_ISSUER` (default: `combyne`)
  - `COMBYNE_AGENT_JWT_AUDIENCE` (default: `combyne-api`)
...
1. In:
   ...
   inject `COMBYNE_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `COMBYNE_API_KEY`, do not overwrite it.
- Continue injecting:
  - `COMBYNE_AGENT_ID`
  - `COMBYNE_COMPANY_ID`
  - `COMBYNE_API_URL`
