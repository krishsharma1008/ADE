...
Add a single storage subsystem for Combyne that supports:
...
- API serves attachment bytes through authenticated Combyne endpoints.
...
- Default local path is under instance root: `~/.combyne-ai/instances/<instanceId>/data/storage`.
...
- `combyne-ai onboard` writes a valid `storage` config block by default.
- `combyne-ai configure --section storage` can switch between local and s3 modes.
...
- `combyne-ai doctor` reports actionable storage status.
