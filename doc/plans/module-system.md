# Combyne Module System

## Overview

Combyne's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Combyne's API, UI, and data model | Yes |
...
modules/
  observability/
    combyne.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@combyne/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@combyne/core";
...
```
1. Scan modules/ directory for combyne.module.json manifests
...
```jsonc
// combyne.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@combyne/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@combyne/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:combyne/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.combyne.ai/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm combyne store list                    # browse available modules and templates
pnpm combyne store install <module-id>     # install a module
pnpm combyne store import <template-id>    # import a company template
pnpm combyne store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Combyne instance | `server:started` |
...
4. **Module config** — `combyne.config.json` with per-module config, env var interpolation
...
10. **`@combyne/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
