...
Agents need API keys to authenticate with Combyne. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Combyne
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Combyne generates a token and passes it directly to the agent
...
`COMBYNE_API_URL`, `COMBYNE_AGENT_ID`, `COMBYNE_COMPANY_ID`. We need to
add a `COMBYNE_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Combyne CLI
...
```
combyne auth login
# Opens browser -> user confirms -> token stored at ~/.combyne-ai/credentials
```
...
managed by the Combyne server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Combyne.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Combyne SKILL.md (or a link to it)
   - What information Combyne needs from the agent (e.g. webhook URL, adapter
...
5. Combyne stores the pending registration.
...
7. On approval, Combyne provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Combyne issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.combyne.ai/skills/combyne/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Combyne should send you heartbeats",
...
1. Generate a Combyne invite link for the company.
...
4. A Combyne company member approves the new agent.
5. Combyne begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `COMBYNE_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `combyne-ai auth login` for developer-managed remote agents.                                      |
