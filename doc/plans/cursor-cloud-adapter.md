...
This document defines the V1 design for a Combyne adapter that integrates with
...
4. synthesized stdout events for Combyne UI/CLI
...
3. **Agent callback auth to Combyne** uses a bootstrap exchange flow (no long-lived Combyne key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Combyne endpoints, not full SKILL.md prompt injection.
...
- `combynePublicUrl` (optional override; else `COMBYNE_PUBLIC_URL` env)
...
## Combyne Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `COMBYNE_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.combynePublicUrl`
2. `process.env.COMBYNE_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Combyne credentials in prompt text.

Flow:

1. Before launch/follow-up, Combyne mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Combyne validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Combyne API calls.
...
1. Prompt includes a compact instruction to fetch skills from Combyne.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/combyne`
   - `GET /api/skills/combyne-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <combynePublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Combyne run context.
...
5. `combynePublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Combyne skill endpoints |
| Combyne auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Combyne context.
3. Add richer PR metadata surfacing in Combyne UI.
