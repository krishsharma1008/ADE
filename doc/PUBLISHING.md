...
Low-level reference for how Combyne packages are built for npm.
...
The CLI package, `combyne-ai`, imports code from workspace packages such as:
...
- `npx combyne@canary onboard` can install them explicitly
- `npx combyne onboard` continues to resolve `latest`
