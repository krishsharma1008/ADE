# ClipHub: Marketplace for Combyne Team Configurations

> The "app store" for whole-company AI teams — pre-built Combyne configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Combyne-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Combyne company (agents, projects, governance) |
...
A complete Combyne company configuration:
...
Single-agent configurations designed to plug into a Combyne org:
...
Portable skill files that any Combyne agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Combyne's skill loading system
...
interface Listing {
  ...
  combyneVersionMin: string;     // Minimum Combyne version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Combyne company
...
| `POST` | `/api/listings/:id/install` | Install to Combyne company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Combyne company (or create new)
  → ClipHub API calls Combyne API to:
  ...
  → Redirect to Combyne dashboard with new team running
```
...
### 5.3 Creator: Export from Combyne → Publish

```
Running Combyne company → "Export as Blueprint" (CLI or UI)
  → Combyne exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Combyne brand blue/purple)
...
| Install flow | One-click deploy to Combyne company |
...
{
  "targetCompanyId": "uuid",         // Existing Combyne company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `combyne-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Combyne
- **Backend**: Node.js API (or extend Combyne server)
- **Database**: Postgres (can share Combyne's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Combyne auth (or OAuth2)

### 9.2 Integration with Combyne

ClipHub can be:
- **Option A**: A separate app that calls Combyne's API to install blueprints
- **Option B**: A built-in section of the Combyne UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Combyne UI and API.
...
- [ ] Install flow: blueprint → Combyne company
...
- [ ] "Export from Combyne" CLI command
