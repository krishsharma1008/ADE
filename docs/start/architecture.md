...
Combyne is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── combyne/               # Core Combyne skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Combyne env vars and a prompt
4. **Agent work** — The agent calls Combyne's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Combyne and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module** — terminal formatter for `combyne-ai run --watch`

Built-in adapters: `claude_local`, `codex_local`, `process`, `http`. You can create custom adapters for any runtime.

## Key Design Decisions

- **Control plane, not execution plane** — Combyne orchestrates agents; it doesn't run them
