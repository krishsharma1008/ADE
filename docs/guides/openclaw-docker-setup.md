...
How to get OpenClaw running in a Docker container for local development and testing the Combyne OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Combyne includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-combyne-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Combyne host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-combyne-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `COMBYNE_HOST_PORT` (default `3100`)
- `COMBYNE_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your Combyne deployment is `authenticated`, provide auth context:

```bash
COMBYNE_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
COMBYNE_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Combyne server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Combyne URL (typically `http://host.docker.internal:3100`).
- If Combyne rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm combyne allowed-hostname host.docker.internal
```

Then restart Combyne and rerun the smoke script.
