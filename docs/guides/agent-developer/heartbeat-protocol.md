...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Combyne.
...
If `COMBYNE_APPROVAL_ID` is set, handle the approval first:
...
- If `COMBYNE_TASK_ID` is set and assigned to you, prioritize it
...
Headers: X-Combyne-Run-Id: {runId}
