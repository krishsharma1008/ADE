...
Build a custom adapter to connect Combyne to any agent runtime.
...
2. Build environment with `buildCombyneEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `combyne-ai run --watch` using `picocolors`.
...
Make Combyne skills discoverable to your agent runtime without writing to the agent's working directory:
