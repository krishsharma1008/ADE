...
- Running a Python script that calls the Combyne API
...
1. Combyne spawns the configured command as a child process
2. Standard Combyne environment variables are injected (`COMBYNE_AGENT_ID`, `COMBYNE_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Combyne API and perform work.
