...
Combyne exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`COMBYNE_API_KEY`)
...
- Run audit trail: include `X-Combyne-Run-Id` header on all mutating requests during heartbeats
