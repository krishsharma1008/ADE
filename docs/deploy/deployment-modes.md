...
Combyne supports two runtime modes with different security profiles.
...
```sh
pnpm combyne allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Combyne emits a one-time claim URL at startup:
...
```sh
COMBYNE_DEPLOYMENT_MODE=authenticated pnpm combyne run
```
