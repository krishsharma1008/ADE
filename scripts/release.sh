...
# release.sh — Prepare and publish a Combyne release.
#
# Stable release:
...
next_canary_version() {
  ...
  versions_json="$(npm view combyne versions --json 2>/dev/null || echo '[]')"
...
if [ "$dry_run" = true ]; then
  ...
  info "Install with: npx combyne@canary onboard"
