...
COMPANY_ID="${COMPANY_ID:-${COMBYNE_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-combyne-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-combyne-openclaw-smoke}"
...
Provide one of:
  COMBYNE_AUTH_HEADER=\"Bearer <board-token>\"
  COMBYNE_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Combyne OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.combyne.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
