...
COMBYNE_HOST_PORT="${COMBYNE_HOST_PORT:-3100}"
COMBYNE_HOST_FROM_CONTAINER="${COMBYNE_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.combyne-openclaw.override.yml"
...
detect_combyne_base_url() {
  ...
  for candidate in "$COMBYNE_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${COMBYNE_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${COMBYNE_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
combyne_base_url="$(detect_combyne_base_url || true)"
...
Combyne URL for OpenClaw container:
EOF
  if [[ -n "$combyne_base_url" ]]; then
    cat <<EOF
  $combyne_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${COMBYNE_HOST_PORT}
  (Do not use http://127.0.0.1:${COMBYNE_HOST_PORT} inside the container.)
  If Combyne rejects the host, run on host machine:
    pnpm combyne allowed-hostname host.docker.internal
  Then restart Combyne and re-run this script.
EOF
  fi
  cat <<EOF
