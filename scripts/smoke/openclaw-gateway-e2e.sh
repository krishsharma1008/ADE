...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-combyne-smoke}"
...
COMBYNE_API_URL_FOR_OPENCLAW="${COMBYNE_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${COMBYNE_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${COMBYNE_AUTH_HEADER}" )
fi
if [[ -n "${COMBYNE_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${COMBYNE_COOKIE}" )
  COMBYNE_BROWSER_ORIGIN="${COMBYNE_BROWSER_ORIGIN:-${COMBYNE_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${COMBYNE_BROWSER_ORIGIN}" -H "Referer: ${COMBYNE_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set COMBYNE_COOKIE or COMBYNE_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg combyneApiUrl "$COMBYNE_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/combyne"
  local claimed_file="${workspace_dir}/combyne-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/combyne-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/combyne-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/combyne-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/combyne"
    assert_status "200"
    {
      echo "COMBYNE_API_URL=${COMBYNE_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set COMBYNE_API_KEY=${AGENT_API_KEY} in your run context before running Combyne heartbeat steps."
...
main() {
  ...
  wait_http_ready "${COMBYNE_API_URL%/}/api/health" 15 || fail "Combyne API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "combyne health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
