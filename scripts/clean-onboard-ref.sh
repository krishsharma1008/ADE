...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Combyne data dir to use
  COMBYNE_HOST=127.0.0.1    Host passed to the onboarded server
  COMBYNE_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/combyne-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/combyne-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export COMBYNE_HOST="${COMBYNE_HOST:-127.0.0.1}"
export COMBYNE_PORT="${COMBYNE_PORT:-3100}"
export COMBYNE_OPEN_ON_LISTEN="${COMBYNE_OPEN_ON_LISTEN:-false}"
...
echo "COMBYNE_HOST: $COMBYNE_HOST"
echo "COMBYNE_PORT: $COMBYNE_PORT"
...
env \
  ...
  HOST="$COMBYNE_HOST" \
  PORT="$COMBYNE_PORT" \
  COMBYNE_OPEN_ON_LISTEN="$COMBYNE_OPEN_ON_LISTEN" \
  pnpm combyne onboard --yes --data-dir "$PC_DATA"
