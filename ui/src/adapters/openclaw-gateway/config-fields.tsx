...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Combyne API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "combyneApiUrl",
                  String(config.combyneApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "combyneApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://combyne.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "combyne"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Combyne persists a device key during onboarding so pairing approvals
