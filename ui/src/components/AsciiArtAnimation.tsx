...
type CombyneSprite = (typeof COMBYNE_SPRITES)[number];
...
function spriteSize(sprite: CombyneSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      // Spell COMBYNE vertically - one letter per row, staggered across columns
      const gapY = 9;
      const gapX = 16;
      for (let baseRow = 1; baseRow < rows - 7; baseRow += gapY) {
        const letterIdx = Math.floor(baseRow / gapY) % COMBYNE_SPRITES.length;
        const startX = (letterIdx % 2 === 0) ? 2 : 8;
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = COMBYNE_SPRITES[letterIdx]!;
    ...
    function spawnClip() {
      // Spawn letters in order: C, O, M, B, Y, N, E — one by one
      const sprite = COMBYNE_SPRITES[nextSpriteIdx % COMBYNE_SPRITES.length]!;
      nextSpriteIdx++;
    ...
    function step(time: number) {
      ...
      // Spawn one letter at a time, sequential C-O-M-B-Y-N-E
      const targetCount = Math.max(1, Math.min(2, Math.floor((cols * rows) / 4000)));
  ...
  return (
    <pre
      ref={preRef}
      className="w-full h-full m-0 p-0 overflow-hidden select-none leading-none"
      style={{ fontSize: "11px", fontFamily: "monospace", color: "oklch(0.75 0.15 85)" }}
      aria-hidden="true"
    />
  );
}
