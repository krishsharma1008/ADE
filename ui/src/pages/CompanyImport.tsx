...
import type { CreateConfigValues } from "@combyne/adapter-utils";
import {
  type FileTreeNode,
            ...
            return (
              <div key={agent.slug}>
                <div className="flex items-center gap-3 px-4 py-2.5 text-sm">
                  ...
                  <ArrowRight className="h-3 w-3 shrink-0 text-muted-foreground" />
                  <select
                    className="min-w-0 flex-1 rounded-md border border-border bg-background text-foreground px-2 py-1 text-xs outline-none focus:border-foreground"
                    value={selectedType}
...
export function CompanyImport() {
  ...
  const localZipHelpText =
    "Upload a .zip exported directly from Combyne. Re-zipped archives created by Finder, Explorer, or other zip tools may not import correctly.";
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Choose a GitHub repo or upload a local Combyne zip package.
          </p>
        </div>
        ...
        <Field label="Target" hint="Import into this company or create a new one.">
          <select
            className="w-full rounded-md border border-border bg-background text-foreground px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            <option value="new" className="bg-background text-foreground">Create new company</option>
            <option value="existing" className="bg-background text-foreground">
        ...
        >
          ...
          >
            <option value="rename" className="bg-background text-foreground">Rename on conflict</option>
            <option value="skip" className="bg-background text-foreground">Skip on conflict</option>
            <option value="replace" className="bg-background text-foreground">Replace existing</option>
          </select>
        </Field>
