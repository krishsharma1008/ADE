...
/**
 * Filter .combyne.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterCombyneYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://combyne.ai) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm combyne company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Combyne](https://combyne.ai) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Combyne](https://combyne.ai) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .combyne.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .combyne.yaml
    const yamlPath = exportData.combyneExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterCombyneYaml(exportData.files[yamlPath], checkedFiles);
    }
